/**
 * The bot check for the site's live assistants (`<talkie-preview>`), as `<talkie-assistant>`'s
 * `verify` hook. The same hook as the virtual tour's `talkie-verify.js`.
 *
 * When the voice server sets TURNSTILE_SECRET_KEY, its POST /agent/session answers
 * `verification_required` with a Cloudflare Turnstile site key, and the assistant calls this hook
 * with `{ provider, siteKey }`. It runs Turnstile and resolves to the token the server checks with
 * siteverify. Nothing loads until the server asks: with the check off, or on the mock backend,
 * Cloudflare's script is never fetched.
 *
 * Turnstile renders into a light-DOM box above the launcher (`.talkie-verify` in theme.css). With
 * appearance "interaction-only" most visitors never see it; a suspicious one gets a checkbox.
...
/**
 * A `verify` hook for `<talkie-assistant>` and `<talkie-preview>`.
 * @param {Document} [doc]
